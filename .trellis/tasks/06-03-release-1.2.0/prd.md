...
* Verification run:
  ...
  * `mvn package "-DskipTests" "-Dmaven.repo.local=D:\Project\Maven\local-maven-repo\repository" -q` in `backend/` passed and generated `backend/target/backend-1.2.0.jar`.
  * `mvn test "-Dmaven.repo.local=D:\Project\Maven\local-maven-repo\repository" -q` in `backend/` passed.
