...
**Git branch policy (required)**: `develop` is the long-lived development branch. Start every new requirement from a fresh branch created off `develop`. Never make a normal development commit directly on `master` or `develop`. Never merge locally into `develop` — push the task branch and open a PR (rebase merge). After PR merges, sync local develop with `git pull --rebase`, then delete the local task branch. Merge `develop` into `master` only for releases. Never delete `develop`.
[/workflow-state:no_task]
...
Git branch policy (required): while planning, stay on `develop` or a task branch created from `develop`. Do not do normal development commits on `master` or `develop`. Never merge locally into `develop` — push the task branch and open a PR (rebase merge).
[/workflow-state:planning]
...
**Git branch policy (required)**: implementation must happen on a task branch created from `develop`, not directly on `master` or `develop`. Never merge locally into `develop` — push the task branch and open a PR (rebase merge). After PR merges, sync local develop with `git checkout develop && git pull --rebase`, then delete the local task branch. Merge `develop` into `master` only for releases. Never delete `develop`.
[/workflow-state:in_progress]
...
**Rules**:
- No `git commit --amend` anywhere — multi-commit flow (work commits → archive commit → journal commit).
- Never push to remote in this step — push happens in `/finish-work`.
- If the user wants different message wording but accepts the file grouping, edit the message and re-confirm once — but if they reject the grouping, exit to manual mode.
- The batched plan is one prompt; do not prompt per commit.

#### 3.5 Wrap-up reminder

After the above, remind the user they can run `/finish-work` to wrap up (archive the task, record the session, push branch, and create PR).
